...
class S3fsTest extends S3fsTestBase {
  ...
  public function testStreamWrapperCoverage() {
    ...
    // Test dir_rewinddir()
    $dir_handler = opendir($this->remoteTestsFolderUri);
    $this->assertNotFalse($dir_handler);
    $initial_dir_list = [];
    $rewound_dir_list = [];
    while (($entry = readdir($dir_handler)) !== FALSE) {
      $initial_dir_list[] = $entry;
    }
    // Add a new file that should be reflected after rewind.
    $this->assertFalse(in_array("rewind_file.txt", $initial_dir_list));
    file_put_contents("{$this->remoteTestsFolderUri}/rewind_file.txt", "Created just before rewind.");
    // Rewind the handler.
    rewind($dir_handler);
    while (($entry = readdir($dir_handler)) !== FALSE) {
      $rewound_dir_list[] = $entry;
    }
    closedir($dir_handler);
    $this->assertNotEmpty($initial_dir_list);
    $this->assertNotEmpty($rewound_dir_list);
    $this->assertCount(count($initial_dir_list) + 1, $rewound_dir_list);
    $this->assertTrue(in_array("rewind_file.txt", $rewound_dir_list));
    unlink("{$this->remoteTestsFolderUri}/rewind_file.txt");
    ...
    $this->assertFalse(is_dir($pathDirExceedMaxLength), 'Verify directory that exceeds max path length does not exist');
    ...
    $this->assertFalse(is_file($pathFileExceedMaxLength), 'File that exceeds max path length does not exist');
  ...
  public function testImageDerivatives() {
    ...
    $new_image_style = ImageStyle::create(['name' => 's3fs_test_image_style', 'label' => 'S3fs Test Style 1']);
    $new_image_style->save();
    $new_image_style_2 = ImageStyle::create(['name' => 's3fs_test_image_style_2', 'label' => 'S3fs Test Style 2']);
    $new_image_style_2->save();

    // Use the "s3fs_test_image_style_2" image style for presigned tests.
    $this->config('s3fs.settings')
      ->set('presigned_urls', "6000|.*/s3fs_test_image_style_2/.*")
      ->save();
    ...
    $style_url_parsed = UrlHelper::parse(ImageStyle::load('s3fs_test_image_style')->buildUrl($img_uri1));
    ...
    $style_presigned_url_parsed = UrlHelper::parse(ImageStyle::load('s3fs_test_image_style_2')->buildUrl($img_uri1));
