  ...
  public function tokenInfo() {
    ...
    foreach ($entities as $entity_type_id => $entity_type) {
      ...
      $info['tokens']['current-page'][$entity_type_id] = [
        'name' => $this->t('The current %type', ['%type' => $entity_type->getLabel()]),
        'description' => $this->t("The current page object if that's a %type", ['%type' => $entity_type->getLabel()]),
        'type' => $this->tokenEntityMapper->getTokenTypeForEntityType($entity_type_id),
      ];
    }
